     ...
     <div class="row">
        <#list groupedField as f>
            ...
            <section class="col col-6">
                <label class="select" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
                    ...
                    <select type="text"
                        ...
                        <option [ngValue]="i" *ngFor="let i of ${Utils.lowerCamel(f.type.name)}List">{{i.name}}
                        </option>
                    </select>
                    <i class="override"></i>
                    <validation-error
            ...
            <section class="col col-6">
                <div class="select" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.type.element)}'].valid && myForm.controls['${Utils.lowerCamel(f.type.element)}'].touched)}">
            ...
            <!-------------------if is switch------------------->
            <section class="col col-6">
                <label class="toggle" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
            ...
            <input type="date"
                   ...
                   placeholder="">
              <section class="col col-6">
                  <label class="input" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
                      ${Utils.spacedCapital(f.name)}<#if f.required> <span style="color: red">*</span> </#if>
            ...
            <div class="">
                <div formArrayName="${Utils.lowerCamel(f.name)}" id="${Utils.lowerCamel(f.name)}">
                    <div *ngFor="let ${Utils.lowerCamel(f.name)} of get${Utils.upperCamel(f.name)}FormData().controls; let i=index" [ngClass]="get${Utils.upperCamel(f.name)}FormData().controls.length > 0 ? 'my-subform-border' : ''">
                        <div [formGroupName]="i">
                            <${Utils.lowerHyphen(f.type.element)}-subform-component
                            [group]="get${Utils.upperCamel(f.name)}FormData().controls[i]"
                            [${Utils.lowerCamel(f.type.element)}]="get${Utils.upperCamel(f.name)}(i)"></${Utils.lowerHyphen(f.type.element)}-subform-component>
            ...
            <#elseif f.richText>
            <div class="" style="margin: 0 15px" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
                ${Utils.spacedCapital(f.name)}<#if f.required> <span style="color: red">*</span> </#if>
            ...
            <div class="row dropzone-area" style="margin: 15px 15px !important">
                <div class="" role="content">
                    <!-- widget content -->
                    <div class="widget-body" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
           ...
           <div class="">
               <section class="col col-12">
                   <label class="textarea" [ngClass]="{'state-error': (!myForm.controls['${Utils.lowerCamel(f.name)}'].valid && myForm.controls['${Utils.lowerCamel(f.name)}'].touched)}">
                       ${Utils.spacedCapital(f.name)}<#if f.required> <span style="color: red">*</span> </#if>
                       <textarea type="text"
                                 rows="4"
                                 cols="200"
